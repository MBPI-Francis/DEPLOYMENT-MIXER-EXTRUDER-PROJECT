...
class SmartComboBox(QComboBox):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # --- NEW: Flag to indicate if the field is mandatory ---
        self._is_mandatory = False

    def set_mandatory(self, mandatory: bool):
        """Set whether this combo box can be empty."""
        self._is_mandatory = mandatory
        self._validate_input() # Re-validate with the new rule

    def is_valid(self) -> bool:
        """Public method to check the current validation state."""
        text = self.currentText()
        if self._is_mandatory and not text.strip():
            return False
        if not text.strip(): # Empty is valid if not mandatory
            return True
        # Valid if the text is in the known list of items
        return text.upper() in self._initial_list_upper_set

    def _validate_input(self):
        """Checks the current text and applies styling."""
        ...
        if is_currently_valid:
            ...
            self.lineEdit().setStyleSheet("background-color: #FFCDD2; border: 1px solid #D32F2F;")
            if self._is_mandatory and not self.currentText().strip():
                self.lineEdit().setToolTip("This field cannot be empty.")
            else:
                self.lineEdit().setToolTip(f"'{self.currentText()}' is not a valid entry.")
