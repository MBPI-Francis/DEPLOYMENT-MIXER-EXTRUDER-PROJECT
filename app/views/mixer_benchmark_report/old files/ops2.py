...
def get_benchmark_data(session: Session, filters: dict):
    ...
    for rec in prod_records:
        ...
        # If range is 1001-1010 (10 lots) and Total Prod is 1000, Unit Weight is 100.
        # Usually T_QTYBATCH stores this Unit Weight, but we verify or fallback.

        unit_weight = float(rec.T_QTYBATCH or 0)

        # Fallback Logic: If QTYBATCH is suspicious, check QTYPROD / Count
        # (Optional: Stick to T_QTYBATCH as per your instruction that it equals Qty/Lot)
    ...
    # -------------------------------------------------------
    # STEP 3: Advanced Resolution (Summing Disjoint Ranges)
    # -------------------------------------------------------
    ...
    def resolve_row(row):
        ...
        # Check if we have source data for this product and suffix
        if p_code in prod_lookup and m_suffix in prod_lookup[p_code]:
            candidates = prod_lookup[p_code][m_suffix]
            ...
            # ITERATE EVERY LOT in the Mixer range (e.g. 2001, 2002, ... 2007)
            # This handles the aggregation of disjoint Prod01 records.
            for current_lot_num in range(m_start, m_end + 1):

                # Find which Prod01 range covers 'current_lot_num'
                matched = False
                for cand in candidates:
                    if cand["start"] <= current_lot_num <= cand["end"]:
                        # Found the source record for this specific lot number
                        ...
                        matched = True
                        break  # Move to next lot number once match found

                # If 'matched' is False, it means 1 lot in the range is missing from Prod01.
                # We simply add 0 to expected output for that missing lot.
            ...
            if len(found_formulas) == 1:
                ...
                # If lots have different formulas, show distinct ones or "Mixed"
    ...
    # Filter by specific Formula (post-resolution)
    if filters.get("formula_no") and filters["formula_no"] != "All":
        # Handle "10678" matching "10678" or "10678;10679"
