...
#         self.EMPTY_TIME_PLACEHOLDER = " --:-- " # The text for our "empty" state
#
...
#
#         self.output_from = QDoubleSpinBox(buttonSymbols=QDoubleSpinBox.ButtonSymbols.NoButtons, maximum=self.MAX_QTY,
#                                           value=current_filters.get("output_qty_from", 0.0))
#         self.output_to = QDoubleSpinBox(buttonSymbols=QDoubleSpinBox.ButtonSymbols.NoButtons, maximum=self.MAX_QTY,
#                                         value=current_filters.get("output_qty_to", self.MAX_QTY))
#         self.cleaning_from = QDoubleSpinBox(buttonSymbols=QDoubleSpinBox.ButtonSymbols.NoButtons, maximum=self.MAX_QTY,
#                                             value=current_filters.get("cleaning_qty_from", 0.0))
#         self.cleaning_to = QDoubleSpinBox(buttonSymbols=QDoubleSpinBox.ButtonSymbols.NoButtons, maximum=self.MAX_QTY,
#                                           value=current_filters.get("cleaning_qty_to", self.MAX_QTY))
#
#         # --- MODIFICATION: Create Time Edits to be "empty" by default ---
#         self.proc_time_from = self._create_empty_time_edit()
#         self.proc_time_to = self._create_empty_time_edit()
#         self.clean_time_from = self._create_empty_time_edit()
#         self.clean_time_to = self._create_empty_time_edit()
#
#         # --- MODIFIED: Adjust the layout to include new fields ---
...
#
#         # --- NEW: Add rows for the new time filters ---
#         form_layout.addWidget(QLabel("<b>Processing Time Range:</b>"), 9, 0)
#         form_layout.addWidget(self.proc_time_from, 9, 1)
#         form_layout.addWidget(QLabel("to"), 9, 2)
#         form_layout.addWidget(self.proc_time_to, 9, 3)
#
#         form_layout.addWidget(QLabel("<b>Cleaning Time Range:</b>"), 10, 0)
#         form_layout.addWidget(self.clean_time_from, 10, 1)
#         form_layout.addWidget(QLabel("to"), 10, 2)
#         form_layout.addWidget(self.clean_time_to, 10, 3)
#
#         main_layout.addLayout(form_layout)
#
#         # --- Buttons Footer (your working code) ---
#         button_layout = QHBoxLayout()
#         button_layout.addStretch(1)
#         self.cancel_button = QPushButton("Cancel")
#         self.apply_button = QPushButton("Apply")
#         self.apply_button.setObjectName("PrimaryDialogButton")
...
#
#     def _create_empty_time_edit(self) -> QTimeEdit:
#         """Helper function to create a QTimeEdit that supports an 'empty' state."""
#         time_edit = QTimeEdit()
#         time_edit.setDisplayFormat("HH:mm")
#         # This is the key: set a special text to display when the time is the minimum (00:00)
#         time_edit.setSpecialValueText(self.EMPTY_TIME_PLACEHOLDER)
#         # Set the time to the minimum, which will make it display the special text
#         time_edit.setTime(QTime(0, 0))
#         return time_edit
#
#     def get_filters(self) -> dict:
#         """
#         Builds a clean dictionary of filters, now including the new time fields.
#         """
#         filters = {}
...
#
#         # Use currentText() for SmartComboBox, not currentIndex
#         if self.mc_name.currentText() != "All Machines":
...
#
#         # --- MODIFICATION: Corrected Time Filter Logic ---
#         # A filter is only added if the displayed text is NOT our placeholder.
#         # This means the user has actively selected a time.
#
#         if self.proc_time_from.text() != self.EMPTY_TIME_PLACEHOLDER:
#             filters["proc_time_from"] = self.proc_time_from.time().toPyTime()
#         if self.proc_time_to.text() != self.EMPTY_TIME_PLACEHOLDER:
#             filters["proc_time_to"] = self.proc_time_to.time().toPyTime()
#
#         if self.clean_time_from.text() != self.EMPTY_TIME_PLACEHOLDER:
#             filters["clean_time_from"] = self.clean_time_from.time().toPyTime()
#         if self.clean_time_to.text() != self.EMPTY_TIME_PLACEHOLDER:
#             filters["clean_time_to"] = self.clean_time_to.time().toPyTime()
#
#         return filters
...
class EditRecordDialog(QDialog):
    ...
    def __init__(self, record_data: pd.Series, machine_list: list, initial_data: dict, parent=None):
        ...
        # --- Create all input widgets (code is unchanged) ---
        self.mc_name = SmartComboBox()
        ...
        self.cleaning_start = QTimeEdit(time=QTime.fromString(str(record_data.get("Cleaning Start", "00:00")), "HH:mm"))
        ...
        # --- MODIFICATION START: Position widgets with section breaks ---

        # --- General Details Section ---
        grid.addWidget(self._create_labeled_widget("Machine", self.mc_name), 0, 0)
        grid.addWidget(self._create_labeled_widget("Code", self.product_code), 0, 1)
        grid.addWidget(self._create_labeled_widget("Lot #", self.lot_number), 0, 2)

        # --- Processing Details Section ---
        processing_header = self._create_section_header("Processing Details")
        grid.addWidget(processing_header, 1, 0, 1, 3)

        grid.addWidget(self._create_labeled_widget("Processed by", self.processed_by), 2, 0, 1, 3)
        grid.addWidget(self._create_labeled_widget("Time Start", self.process_start), 3, 0)
        grid.addWidget(self._create_labeled_widget("Time End", self.process_end), 3, 1)
        grid.addWidget(self._create_labeled_widget("Output QTY", self.output_qty), 3, 2)

        # --- Cleaning Details Section ---
        cleaning_header = self._create_section_header("Cleaning Details")
        grid.addWidget(cleaning_header, 4, 0, 1, 3)

        grid.addWidget(self._create_labeled_widget("Cleaning RM", self.cleaning_rm), 5, 0, 1, 3)
        grid.addWidget(self._create_labeled_widget("Time Start", self.cleaning_start), 6, 0)
        grid.addWidget(self._create_labeled_widget("Time End", self.cleaning_end), 6, 1)
        grid.addWidget(self._create_labeled_widget("Cleaning QTY", self.cleaning_qty), 6, 2)

        # --- Remarks Section ---
        grid.addWidget(self._create_labeled_widget("Remarks", self.remarks), 7, 0, 1, 3)
    ...
    def validate_data(self) -> bool:
        ...
        # if not self.lot_number.is_valid():
        #     QMessageBox.warning(self, "Validation Error", "Lot Number is invalid or empty.")
        #     self.lot_number.setFocus();
        #     return False

        if not self.mc_name.is_valid():
            QMessageBox.warning(self, "Validation Error", "Machine Number is invalid or empty.")
            self.lot_number.setFocus();
            return False
        ...
        if not self.cleaning_rm.is_valid():
            QMessageBox.warning(self, "Validation Error", "Cleaning RM is invalid or empty.")
            self.lot_number.setFocus();
            return False
    ...
    def get_updated_data(self) -> dict:
        # This data collection is also correct and remains unchanged
        return {"MC #": self.mc_name.currentText(), "product_code": self.product_code.text().strip(),
                "lot_no": self.lot_number.text().strip(), "process_time_start": self.process_start.time().toPyTime(),
                "process_time_end": self.process_end.time().toPyTime(),
                "processed_by": self.processed_by.text().strip(), "output_qty": self.output_qty.value(),
                "cleaning_time_start": self.cleaning_start.time().toPyTime(),
                "cleaning_time_end": self.cleaning_end.time().toPyTime(),
                "cleaning_rm_code": self.cleaning_rm.text().strip(), "cleaning_qty": self.cleaning_qty.value(),
