...
# --- Worker and Dialog classes are efficient and do not need changes ---
...
class MixerRecordsView(QWidget):
    ...
    def _append_to_table(self, data: pd.DataFrame):
        ...
        for i, row in enumerate(data.itertuples(index=False), start=start_row):
            for j, col_name in enumerate(headers):
                # --- THIS IS THE FIX for missing data ---
                # itertuples mangles names with spaces/symbols. We must account for this.
    ...
    def _handle_export_to_excel(self):
        ...
        # OPTIMIZATION: Use the full_data DataFrame directly. It's much faster.
    ...
    # ... (The rest of the file, including dialogs, context menus, etc., remains the same)
    def load_data(self):
