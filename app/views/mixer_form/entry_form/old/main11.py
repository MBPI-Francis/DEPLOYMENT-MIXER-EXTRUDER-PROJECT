...
# --- NEW CLASS: FormattedDoubleSpinBox ---
class FormattedDoubleSpinBox(QDoubleSpinBox):
    """A QDoubleSpinBox that displays values with thousands separators."""
    ...
    def textFromValue(self, value: float) -> str:
        """Overrides the default text display to format with commas."""
        return f"{value:,.2f}"
    

# --- NEW CLASS: RemarksDialog ---
class RemarksDialog(QDialog):
    """A dedicated, styled dialog for entering and editing long remarks."""
    def __init__(self, current_remarks: str = "", parent=None):
        super().__init__(parent)
        self.setWindowTitle("Edit Remarks")
        self.setMinimumWidth(450)
        self.setModal(True)
        # Apply the same modern stylesheet to this dialog
        if parent:
    ...
    def get_remarks(self) -> str:
        """Returns the text from the text editor."""
...
class DetailRowWidget(QWidget):
    ...
    def __init__(self, machine_map: dict, detail=None):
        ...
        # --- MODIFIED: Remarks is now a button styled like a link ---
        self.remarks_button = QPushButton()
        ...
        self.update_remarks_button_text() # Set initial text
        
        if detail:
            self.machine_combo.setCurrentIndex(self.machine_combo.findData(detail.mc_id))

        # --- NEW: Time duration labels and connections ---
        ...
        # layout.addWidget(self.remarks_button, 3) # Add the button to the layout
        ...
        layout.addWidget(self.lot_count_spin, 2)
        layout.addWidget(proc_time_widget, 3)
        layout.addWidget(self.proc_time_label, 1) # Add the label
        layout.addWidget(self.processed_by_edit, 3)
        layout.addWidget(self.output_qty_spin, 2)
        layout.addWidget(clean_time_widget, 3)
        layout.addWidget(self.clean_time_label, 1) # Add the label
        layout.addWidget(self.clean_rm_code_edit, 3)
        layout.addWidget(self.clean_qty_spin, 2)
    ...
    def _calculate_time_diffs(self):
        """Calculates and displays the difference between start and end times."""
    ...
    def _update_single_time_diff(self, start_widget: TimeLineEdit, end_widget: TimeLineEdit, label: QLabel):
        """Helper to calculate and format the time difference for a given pair of widgets."""
        try:
    ...
    def update_remarks_button_text(self):
        """Updates the text of the remarks button based on whether it has content."""
    ...
    def open_remarks_dialog(self):
        """Opens a dialog to edit the remarks for this specific row."""
        dialog = RemarksDialog(self._remarks_text, self)
        if dialog.exec(): # This blocks until the user clicks Save or Cancel
    ...
    def get_data(self) -> MixerDetailCreateValidator:
        ...
        return MixerDetailCreateValidator(
            ...
            remarks=self._remarks_text # Get remarks from the internal variable
        )
...
class MixerFormView(QWidget):
    """
    The main content widget for the Mixer Form.
    """
    ...
    def keyPressEvent(self, event: QKeyEvent):
        """
        Overrides the keyPressEvent to handle global shortcuts for this widget.
        """
    ...
    def showEvent(self, event):
        ...
        if self.isVisible():
            # This check prevents unnecessary refreshes
            if not self.machine_map: 
                self._populate_machine_combobox()

            # The rest of the logic can be simplified if we rebuild rows
            # during the _load_draft_from_db call
            self._load_draft_from_db()