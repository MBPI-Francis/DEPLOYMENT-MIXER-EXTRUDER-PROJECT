...
from PyQt6.QtCore import pyqtSignal, Qt, QDate, QEvent, QThread, QStringListModel, pyqtSlot
...
class MixerFormView(QWidget):
    ...
    def _load_initial_data(self):
        ...
        on the main UI thread without a background worker.
        """
        print("Loading initial form data...")
        self.setEnabled(False)
        self.staged_group.setTitle("Loading options, please wait...")
        QApplication.processEvents() # Allow the UI to update to show the message
        ...
        try:
            ...
            # 2. Load legacy data (these DB queries are fast)
    ...
    def _add_row_widget(self, detail=None):
        ...
        # Populate the fast combo boxes AFTER creation using the master lists
        row_widget.lot_no_edit.populate(self.lot_number_list)
        row_widget.product_code_edit.populate(self.product_code_list)
