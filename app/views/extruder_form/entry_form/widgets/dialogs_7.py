...
class LotNumberDialog(QDialog):
    ...
    def _apply_selections(self):
        """
        Validates the current state and sends the selected data back to the main form.
        If this is the first lot being applied, it immediately refreshes the dialog
        into its permanently filtered state.
        """
        ...
        if self.initial_product_code_lock and self.initial_customer_lock:
            ...
            if (current_lot_pc != self.initial_product_code_lock or
                ...
                QMessageBox.critical(self, "Validation Error",
                                     "The selected lot does not match the required Product Code and Customer.")
                return
        ...
        # 1. Send data back to the main form
        ...
        # 2. If it was the first apply, reconfigure the dialog into its permanent filtered state
        if is_initial_apply:
        ...
        # 3. Always reset the temporary selections for the next action
    ...
    def _on_lot_double_clicked(self, item: QListWidgetItem):
        """Handles double-clicking a lot: This is the explicit action to lock a lot."""
        ...
        # Auto-move formula if there's only one
    ...
    def _add_formula_to_selection(self):
        """
        Handles the 'Add Formula' button click. This is the implicit action to lock a lot
        if no lock is currently active.
        """
