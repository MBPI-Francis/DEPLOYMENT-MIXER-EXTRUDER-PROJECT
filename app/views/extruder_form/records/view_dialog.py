...
class ExtruderRecordViewDialog(QDialog):
    ...
    def _create_purging_resin_group(self):
        ...
        self.resin_table.setEditTriggers(QTableWidget.EditTrigger.NoEditTriggers)
        r_layout.addWidget(self.resin_table)
        h_layout.addWidget(purging_sub);
        h_layout.addWidget(resin_sub)
        layout.addLayout(h_layout)
        return group
    ...
    #     D = Decimal
    #     qty_order = D(record.qty_order or 0).quantize(D('0.01'), rounding=ROUND_HALF_UP)
    ...
    #         total_hours = D(total_seconds) / D(3600)
    #
    #     output_per_hour = (total_output / total_hours) if total_hours > 0 else D(0)
    #
    #     # Gain/Loss/Percentage calculations
    #     gain = max(D(0), total_output - qty_order)
    #     loss = max(D(0), qty_order - total_output)
    #
    #     output_percent = (total_output / qty_order * 100) if qty_order > 0 else D(0)
    #     gain_percent = (gain / qty_order * 100) if qty_order > 0 else D(0)
    #     loss_percent = (loss / qty_order * 100) if qty_order > 0 else D(0)
    #
