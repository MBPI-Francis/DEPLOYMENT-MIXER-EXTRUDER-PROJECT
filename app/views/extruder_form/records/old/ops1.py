...
class ExtruderRecordsOperations:
    ...
    def get_records_with_details(self, search_term=None, date_from=None, date_to=None, show_deleted=False):
        ...
        eagerly loaded, but without complex subqueries. Calculations will be done in Python.
        """
        session = self.Session()
        try:
            query = session.query(ExtruderFormData)

            # Use eager loading to get all related data in an efficient way
            query = query.options(
            ...
            # This now returns a list of fully-loaded ExtruderFormData objects
            return query.all()
