...
class ExtruderRecordsView(QWidget):
    ...
    def _add_record_to_table(self, record):
        ...
        cell_data = {
            ...
            2: (record.completion_date.strftime("%Y-%m-%d %H:%M") if record.completion_date else "N/A",
                record.completion_date.timestamp() if record.completion_date else 0),
            3: (str(record.ref_no or ''), record.ref_no),
            4: (getattr(record.machine, 'name', 'N/A'), None),
            5: (record.product_code, None),
            6: (record.lot_number, None),
            7: (min_start_time.strftime("%Y-%m-%d %H:%M") if min_start_time else "N/A",
                min_start_time.timestamp() if min_start_time else 0),
            8: (max_end_time.strftime("%Y-%m-%d %H:%M") if max_end_time else "N/A",
                max_end_time.timestamp() if max_end_time else 0),
            9: (f"{output_per_hour:.2f}", float(output_per_hour)),
            10: (f"{record.target_output_per_hour or 0:.2f}", float(record.target_output_per_hour or 0)),
            11: (f"{total_output or 0:.2f}", float(total_output or 0)),
            12: (", ".join([p.product_code for p in record.purging_headers if p.product_code]) or "N/A", None),
            13: (purging_time_str, total_purging_seconds),
            14: (", ".join([f"{p.employee.first_name} {p.employee.last_name}" for p in record.extruder_personnels if
