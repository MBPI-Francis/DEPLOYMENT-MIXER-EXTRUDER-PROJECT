...
#         """
#         Efficiently appends a DataFrame to the end of the QTableWidget.
#         This version robustly handles column mismatches.
#         """
#         self.table.setUpdatesEnabled(False)
#         self.table.setSortingEnabled(False)
#
...
#         numeric_cols = {"Ref No", "Output QTY", "Cleaning QTY"}
#
#         # Use iterrows here, as it provides direct access by column name,
#         # which is more robust for this specific problem.
#         for i, (index, row) in enumerate(data.iterrows(), start=start_row):
#             for j, col_name in enumerate(headers):
#
#                 # --- THIS IS THE DEFINITIVE FIX ---
#                 # Get data from the DataFrame 'row' using the column name from the UI header.
#                 # The .get() method safely handles cases where a column might be missing.
...
#         # --- START OF FIX ---
#
#         # 1. Restore the intended stretch columns. These will fill any extra space.
#         stretch_columns = {}
#
#         for i, h in enumerate(headers):
#             if h in stretch_columns:
#                 # This mode makes the column expand to fill available horizontal space.
#                 header.setSectionResizeMode(i, QHeaderView.ResizeMode.Stretch)
#             else:
#                 # This is the key change: Automatically resize columns to fit their content.
...
#     def refresh_data(self, is_manual_refresh: bool = True):
#         """
#         Resets and reloads data. Decides whether to use lazy loading or
#         load all data based on whether filters are active.
#         """
#         self.current_offset = 0
#         self.data_frames.clear()
#         self._all_data_loaded = False
...
#             # --- NEW LOGIC: FILTERS ARE ACTIVE ---
#             print("Filters are active. Loading all matching data...")
#             self._is_loading_more = True  # Block scrolling
#             self._all_data_loaded = True  # No more pages to load
#
#             session = self.Session()
#             try:
#                 # Call without offset/limit to get all data
#                 all_filtered_data = get_mixer_report_data(session, self.current_filters)
#                 if not all_filtered_data.empty:
#                     self.data_frames.append(all_filtered_data)
#                     self.full_data = pd.concat(self.data_frames, ignore_index=True)
#                     self._append_to_table(all_filtered_data)
#                     self._update_summary_box()
#             finally:
#                 session.close()
#                 self._is_loading_more = False
#         else:
#             # --- EXISTING LOGIC: NO FILTERS, USE LAZY LOADING ---
#             self._load_more_data()
#
#         if is_manual_refresh:
#             self.show_notification("Data successfully refreshed!")
...
#         scrollbar = self.table.verticalScrollBar()
#         if value == scrollbar.maximum() and not self._is_loading_more and not self._all_data_loaded:
#             # This will only be triggered now if no filters are active
#             self._load_more_data()
#
#     def _load_more_data(self):
#         if self._is_loading_more: return
...
#             if ':' in duration_str:
#                 try:
#                     hours, minutes = map(int, duration_str.split(':')); total_proc_delta += timedelta(hours=hours,
#                                                                                                       minutes=minutes)
#                 except (ValueError, TypeError):
#                     continue
#         total_clean_delta = timedelta()
...
#             if ':' in duration_str:
#                 try:
#                     hours, minutes = map(int, duration_str.split(':')); total_clean_delta += timedelta(hours=hours,
#                                                                                                        minutes=minutes)
#                 except (ValueError, TypeError):
#                     continue
#         proc_decimal_hours = total_proc_delta.total_seconds() / 3600
