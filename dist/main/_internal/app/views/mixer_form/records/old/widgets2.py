...
class EditRecordDialog(QDialog):
    ...
    def __init__(self, record_data: pd.Series, machine_list: list, initial_data: dict, parent=None):
        ...
        self.process_start = QTimeEdit(time=QTime.fromString(str(record_data.get("Processing Start", "00:00")), "HH:mm"))
        ...
        self.cleaning_qty = QDoubleSpinBox(maximum=999999, decimals=2, value=float(record_data.get("Cleaning QTY", 0.0)))
    ...
    def validate_data(self) -> bool:
        ...
        if (rm_qty > 0 and not rm_code):
            ...
            self.cleaning_rm.setFocus(); return False
    ...
    def get_updated_data(self) -> dict:
        ...
        return {"MC #": self.mc_name.currentText(), "product_code": self.product_code.text().strip(), "lot_no": self.lot_number.text().strip(), "process_time_start": self.process_start.time().toPyTime(), "process_time_end": self.process_end.time().toPyTime(), "processed_by": self.processed_by.text().strip(), "output_qty": self.output_qty.value(), "cleaning_time_start": self.cleaning_start.time().toPyTime(), "cleaning_time_end": self.cleaning_end.time().toPyTime(), "cleaning_rm_code": self.cleaning_rm.text().strip(), "cleaning_qty": self.cleaning_qty.value(), "remarks": self.remarks.toPlainText().strip()}
