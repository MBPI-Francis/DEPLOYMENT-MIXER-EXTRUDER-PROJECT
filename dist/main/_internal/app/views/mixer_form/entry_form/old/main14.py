...
class MixerFormView(QWidget):
    def __init__(self, session_factory: Type[sessionmaker], parent=None):
        ...
        # --- CRITICAL FIX: Create and OWN the models in the main thread. ---
        # They are parented to the view itself.
    ...
    def _load_initial_data(self):
        ...
        # The worker is parented to the view and receives the view's models
        self.data_loader = DataLoaderWorker(self.Session, self.lot_number_model, self.product_code_model, self)
        self.data_loader.finished_loading.connect(self._on_legacy_data_loaded) # Connect to new signal
    ...
    def _add_row_widget(self, detail=None):
        # The models are now guaranteed to exist, but they might still be empty
        # while the worker is running. The setEnabled(False) prevents this.
