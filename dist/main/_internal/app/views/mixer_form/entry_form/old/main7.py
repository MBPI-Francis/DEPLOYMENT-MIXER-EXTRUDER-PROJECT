...
class MixerFormView(QWidget):
    ...
    def eventFilter(self, source, event):
        ...
        This now works from any field inside any DetailRowWidget.
        """
        if event.type() == QEvent.Type.KeyPress:
            if event.key() == Qt.Key.Key_Return and (event.modifiers() & Qt.KeyboardModifier.ShiftModifier):
                ...
                if isinstance(current_focus, QWidget):
                    # Check if the focused widget is a child of any of our DetailRowWidgets
                    for row_widget in self.detail_rows:
                        if current_focus.parentWidget() is row_widget:
                            self._handle_add_row()
                            return True # True means we've handled the event
        # If it's not our specific shortcut, pass the event on
