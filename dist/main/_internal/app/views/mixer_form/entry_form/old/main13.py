...
from app.widgets.fast_combo_box import FastComboBox
...
class DataLoaderWorker(QThread):
    models_ready = pyqtSignal(QStringListModel, QStringListModel)
    error = pyqtSignal(str)

    def __init__(self, session_factory: Type[sessionmaker], parent=None):
        super().__init__(parent)
        self.Session = session_factory
        # This parenting is the key to preventing crashes
        self.lot_number_model = QStringListModel(self)
        self.product_code_model = QStringListModel(self)
    ...
    def run(self):
        ...
        try:
            ...
            self.models_ready.emit(self.lot_number_model, self.product_code_model)
...
class DetailRowWidget(QWidget):
    ...
    def __init__(self, machine_map: dict, lot_number_model: QStringListModel, 
                 product_code_model: QStringListModel, detail=None):
        super().__init__()
        ...
        self.product_code_edit = FastComboBox()
        self.product_code_edit.set_shared_model(product_code_model)
        self.product_code_edit.setText(getattr(detail, 'product_code', ''))
        
        self.lot_no_edit = FastComboBox()
        self.lot_no_edit.set_shared_model(lot_number_model)
        self.lot_no_edit.setText(getattr(detail, 'lot_no', ''))
...
class MixerFormView(QWidget):
    def __init__(self, session_factory: Type[sessionmaker], parent=None):
        ...
        self.lot_number_model = None
        self.product_code_model = None
    ...
    def _load_initial_data(self):
        ...
        # Keep the worker as an instance attribute
        self.data_loader = DataLoaderWorker(self.Session, self)
        self.data_loader.models_ready.connect(self._on_legacy_data_loaded)
        self.data_loader.error.connect(self._on_data_load_error)
        self.data_loader.start()

    @pyqtSlot(QStringListModel, QStringListModel)
    def _on_legacy_data_loaded(self, lot_number_model, product_code_model):
        print("UI Thread: Received prepared models from worker.")
        self.lot_number_model = lot_number_model
        self.product_code_model = product_code_model
    ...
    def _load_draft_from_db(self):
        ...
        try:
            ...
            if draft and draft.details:
                ...
                self._add_row_widget() # Add one blank row if no draft exists
        finally:
            session.close()

    def _add_row_widget(self, detail=None):
        if self.lot_number_model is None or self.product_code_model is None:
            return
        row_widget = DetailRowWidget(
            self.machine_map, 
            self.lot_number_model, 
            self.product_code_model, 
            detail
        )
    ...
    def showEvent(self, event):
        ...
        if self.isVisible():
            if self.lot_number_model is None:
                self._load_initial_data()