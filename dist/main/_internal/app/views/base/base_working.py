...
# --- ADD THIS IMPORT ---
# Import the new controller class from your features module.
...
class Base(QMainWindow):
    ...
    def __init__(
        ...
        # --- NEW ---
        # Initialize the sync controller once.
    ...
    def _create_top_bar(self) -> QWidget:
        ...
        self.sync_db_button.clicked.connect(self.start_db_sync) # Connect to the new method
        ...
        # ... (rest of the top bar code is identical)
    ...
    # --- ADD THIS NEW METHOD TO THE Base CLASS ---
    def start_db_sync(self):
        """
        Starts the database synchronization process by calling the controller.
        Also handles enabling/disabling the button.
        """
        self.sync_db_button.setEnabled(False)
        self.sync_db_button.setText(" Syncing...")

        self.sync_controller.run_sync()

        # The controller will now handle the dialog. We just need to re-enable the button when it's done.
        # This is a simple way to do it. For more complex apps, you might use a signal.
        self.sync_controller.sync_worker.finished.connect(lambda: self.sync_db_button.setEnabled(True))
        self.sync_controller.sync_worker.finished.connect(lambda: self.sync_db_button.setText(" Sync Legacy DB"))
        self.sync_controller.sync_worker.error.connect(lambda: self.sync_db_button.setEnabled(True))
        self.sync_controller.sync_worker.error.connect(lambda: self.sync_db_button.setText(" Sync Legacy DB"))