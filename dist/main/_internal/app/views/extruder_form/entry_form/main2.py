...
class ExtruderEntryFormView(QWidget):
    ...
    #
    #     #   ADD INITIAL ROW FOR EXTRUDER OUTPUT LOG
    ...
    # def eventFilter(self, obj: QObject, event: QEvent) -> bool:
    #     """
    #     Catches keyboard shortcuts for the entire form, including the new
    #     shortcuts for the output log table.
    #     """
    #     if event.type() == QEvent.Type.KeyPress:
    #         # --- NEW: Shortcut for Deleting a row (Ctrl + D) ---
    #         if event.key() == Qt.Key.Key_D and event.modifiers() == Qt.KeyboardModifier.ControlModifier:
    #             # Check if the focus is inside the output log table
    #             if self.ui.output_log_table.hasFocus():
    ...
    #
    #         # --- NEW: Shortcut for Adding a new row (Shift + Enter) ---
    #         # Using Key_Return for Enter on the main keyboard, Key_Enter for the numpad
    ...
    #                 return True # Event handled
    #
    #         # --- Existing logic for skipping the "Time Used" column ---
    ...
    #     """
    #     # --- THIS IS THE FIX ---
    #     # 1. Check if the event is a key press AND if the object receiving
    #     #    the event is the output log table's viewport.
    #     if event.type() == QEvent.Type.KeyPress and obj is self.ui.output_log_table.viewport():
    #
    #         # 2. Now that we know we're in the right table, check the specific key combinations.
    #
    ...
    #         if event.key() in (
    #         Qt.Key.Key_Return, Qt.Key.Key_Enter) and event.modifiers() == Qt.KeyboardModifier.ShiftModifier:
    #             self._add_output_log_row()
    #             return True  # Event handled
    #
    #     # --- The existing Tab key logic for the Time End widget is separate and still correct ---
