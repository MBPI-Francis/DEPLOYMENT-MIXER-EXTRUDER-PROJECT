...
class ExtruderRecordsView(QWidget):
    ...
    def refresh_data(self):
        self._load_records()
        # --- NEW: Update UI state every time data is refreshed ---
        self._update_ui_for_view_mode()

    def load_initial_data(self):
        self._load_records()

    # --- NEW: Method to manage UI visibility ---
    def _update_ui_for_view_mode(self):
        """Shows or hides controls based on the current view (active vs. deleted)."""
        is_deleted_view = self.ui.show_only_deleted_checkbox.isChecked()
        self.ui.restore_selected_button.setVisible(is_deleted_view)

    # --- NEW: Handler for the Restore Selected button ---
    def _restore_selected_records(self):
        """Gathers selected record IDs and triggers a bulk restore operation."""
    ...
    def _load_records(self):
        ...
        try:
            ...
            # This line correctly passes the checkbox state to the ops layer
    ...
    def _show_context_menu(self, position: QPoint):
        ...
        if is_deleted:
            # If viewing deleted records, show both View and Restore
            view_action = QAction("View Record", self)
            restore_action = QAction("Restore Record", self)
    ...
    def _view_record(self):
        ...
        if record_id is None:
            return  # Only guard against no selection
